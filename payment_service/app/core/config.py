...
class Settings:
    ...
    DB_URL_ASYNC: str = os.getenv(
        "PAYMENT_DATABASE_URL_ASYNC", "sqlite+aiosqlite:///./test.db"
    )
    DB_URL_SYNC: str = os.getenv("PAYMENT_DATABASE_URL_SYNC", "sqlite:///./test.db")
    ...
    @property
    def BOOKING_GRPC_HOST(self) -> str:
        return (
            f"{self.GRPC_HOST_FOR_BOOKING_SERVICE}:{self.GRPC_PORT_FOR_BOOKING_SERVICE}"
        )

    # KAFKA SETTINGS
    KAFKA_BOOTSTRAP_SERVERS: str = os.getenv("KAFKA_BOOTSTRAP_SERVERS", "kafka:9092")
    KAFKA_TOPIC_LOGS: str = os.getenv("KAFKA_TOPIC_LOGS", "logs")
    KAFKA_TOPIC_NOTIFICATIONS: str = os.getenv(
